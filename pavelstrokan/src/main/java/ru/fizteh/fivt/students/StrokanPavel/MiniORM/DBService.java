...
import ru.fizteh.fivt.students.StrokanPavel.MiniORM.Annotations.Table;
...
public class DBService<T> {
    ...
    public DBService(Class<T> givenClass) throws ClassNotFoundException, SQLException {
        ...
        tableName = givenClass.getAnnotation(Table.class).name();
        allFields = givenClass.getDeclaredFields();
        if (tableName.equals("")) {
            tableName = givenClass.getSimpleName();
